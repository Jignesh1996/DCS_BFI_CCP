...
rho2 = 25; %mm distance for long channel
...
%% Applying 3 layer model to estimate CBF
...
%     F1 = aDb(1,i);
    ...
    LB = [1e-15 0  1e-15];
    UB = [1e-5 0 1e-5];
    Starting = [1e-9 0 1e-9]; %[Fc; cm^2/s]
    ...
    Fscalp(j) = FittedParams(1);
    Fskull(j) = FittedParams(2);
    Fbrain(j) = FittedParams(3);
    %     DeltaFec(subject) = FittedParams(3);
    close all
    j=j+1;
