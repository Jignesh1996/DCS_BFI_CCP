...
%% Finding the minima to find the starting of the signal
sg_lp_30 = dcs3_lp(1510:2110).*10^9;
% sg_lp_30(109:112) = [];
% sg_lp_30(393:441) = [];
% sg_lp_30(439:443) = [];
minima = islocalmin(sg_lp_30,'MinSeparation',12,'MinProminence',1);
x = 1:length(minima);
figure();
...
%% Plotting the data based on the minima
ini = sg_lp_30(9:25);
cyc =zeros(sum(minima==1)-1,17); 
cyc(1,:)= ini;
count = 1;
avg = ini;
for i=26:1:length(minima)
    if (minima(i)==1) && (i+16<=length(minima))
        count = count+1;
